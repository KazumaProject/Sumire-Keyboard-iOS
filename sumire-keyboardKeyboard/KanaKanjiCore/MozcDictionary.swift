...
public struct MozcDictionary: Sendable {
    ...
    func predictiveEntries(
        ...
        guard limit > 0, input.isEmpty == false else {
            return []
        }
        ...
        case let .memory(trie, entryGroups):
            ...
            let k = max(1, min(predictivePrefixLength, characters.count))
            let prefix = String(characters.prefix(k))
            var entries: [DictionaryEntry] = []
            entries.reserveCapacity(limit * 2)
            let yomiLimit = max(limit, 200)

            for result in trie.predictiveSearch(
                prefix: prefix,
                matching: input,
                limit: yomiLimit,
                maxKeyLength: maxYomiLength
            ) {
                guard result.value >= 0, result.value < entryGroups.count else {
                    continue
                }
                for entry in entryGroups[result.value] {
                    entries.append(entry)
                }
            }
            return Self.sortedPredictiveEntries(entries).prefix(limit).map { $0 }
        case let .artifacts(artifacts):
            return artifacts.predictiveEntries(
                for: input,
                predictivePrefixLength: predictivePrefixLength,
                limit: limit,
    ...
    static func sortedPredictiveEntries(_ entries: [DictionaryEntry]) -> [DictionaryEntry] {
        entries.sorted {
            if $0.yomi.count != $1.yomi.count {
                return $0.yomi.count > $1.yomi.count
            }
            if $0.cost != $1.cost {
                return $0.cost < $1.cost
            }
